...
% To run this code:
% 1. Adjust 'featurePath' to match corresponding user directory which holds
% previous preprocessed data
% 2. change 'dataName' to corresponding data 
% 3. Adjust 'featureName' between 'RAW' or 'FTA'
% 4. Adjust 'frequencyRange' to Either _4 or _30 or _8
clear; close all; clc;

featurePath = 'C:\Users\jki342.AD\EEG_Codes\results\Thapa_0.5_30\Thapa_0.5_30';
%  dataName = 'FREEFORMSubjectB1511112StLRHand';
%  dataName = 'FREEFORMSubjectC1512082StLRHand';
%   dataName = 'FREEFORMSubjectC1512102StLRHand';
 dataName = 'sub-23_ses-02';

featureName = 'RAW'; %Choose the feature name either 'RAW' or 'FTA'
...
frequencyRange = '_0.5_4'; %Either _4 or _30 or _8
classifierName = 'KTD';

testRunName = '';

codePath = pwd;
featurePath = [featurePath '\' dataName '\Data_from_' num2str(tstart) '_to' num2str(tend)]; 
folderName = [classifierName 'source' featureName frequencyRange];
resultPath = fullfile(featurePath,folderName); 
if exist(resultPath,'dir')~= 7,mkdir(resultPath),end

dataFileName = ['EEGfeature' featureName frequencyRange '.mat'];
if ~exist(fullfile(featurePath,dataFileName),'file'),disp([dataFileName ' does not exist.']),end
load(fullfile(featurePath,dataFileName));

classID = classID(:);
classID = double(classID); %ensuring data for Thapa preprocessing is in the correct format (column array and double)
...
parms.nepoch = 50; % number of epoches
parms.nMCrun = 10; % numger of Monte Carlos runs
% parms.stopThr = 0.01; %Threshhold for convergence
...
% RL parameters
RLepsilonInitial = 0.01; 
parms.RLepsilon = RLepsilonInitial; % exploration rate for the epeilson greedy method
...
saveas(h,fullfile(resultPath,'Fig_TargetXY.tif'))
saveas(h,fullfile(resultPath,'Fig_TargetXY.fig'))
...
% TLunit = cell(1,parms.nMCrun);
% TLweight = cell(1,parms.nMCrun);
% TLepochLength = cell(1,parms.nMCrun);

for imcrun = 1:parms.nMCrun
    ikernel = 1;   
    parms.RLepsilon = RLepsilonInitial;
            ...
            if iepoch == 1 && itrial == 1
                ...
                if isempty(quantizationIndex)
                    ...
                    if length(quantizationIndex)>1
                        fprintf('=====================ERROR========================\n')
                        fprintf('Quantization Weight! Please modify your quantization threshold.\n')
                        pause
                    else
                ...
                if cursorDistance < (parms.ReachingDisanceThr)
...
%     TLunit{1,imcrun} = Unit;
%     TLweight{1,imcrun} = Weight;
%     TLepochLength{1,imcrun} = iepoch;
...
save(fullfile(resultPath,['ResultsWparms' testRunName '.mat']), ...
    ...
    'successRate') 
% removed... 'TLunit','TLweight','TLepochLength'
